...
//计算某个商品总价
function countItemSum(itemObj,key){ // key:0,表示计算折扣价格，key：1，计算正常价格
...
//计算清单总价格
function calculateTotalListPriceMoney(productList, key){ // key:0,表示计算折扣价格，key：1，计算正常价格
    let sum = 0
...
function printReceipt(productArray){
    ...
    let goodsList = matchProductInfo(calculationArray) // 信息匹配和计算总价后的数组
    let normalTotalMoney = calculateTotalListPriceMoney(goodsList, 1) //计算正常总价格
    let discountTotalMoney = calculateTotalListPriceMoney(goodsList, 0) //计算打折总价格
